...
def calculate_distance(point1, point2):
        # Check if point1 is a tuple or an integer
        if isinstance(point1, tuple):
            x1, y1 = point1
        elif isinstance(point1, int):  
            x1, y1 = point1, 0
        else:
            # Handle other cases or raise an exception
            raise ValueError("Unsupported type for point1")

        # Check if point2 is a tuple or an integer
        if isinstance(point2, tuple):
            x2, y2 = point2
        elif isinstance(point2, int):
            x2, y2 = point2, 0
        else:
            # Handle other cases or raise an exception
            raise ValueError("Unsupported type for point2")
